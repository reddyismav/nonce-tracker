const { checkExitTransactionIfReplaced } = require('../services/root-exit')

export const callCheckExitTransactionIfReplaced = async(req, res) => {
  try {
    // All your validation can be done here before calloing the main function
    const { query, params } = req
    const reqParams = { query, params }
    const result = await checkExitTransactionIfReplaced(reqParams)
