const { checkDepositTransactionIfReplaced } = require('../services/root-deposit')

export const callCheckDepositTransactionIfReplaced = async(req, res) => {
  try {
    // All your validation can be done here before calloing the main function
    const { query, params } = req
    const reqParams = { query, params }
    const result = await checkDepositTransactionIfReplaced(reqParams)
